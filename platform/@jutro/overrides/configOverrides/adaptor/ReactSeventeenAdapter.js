...
const { typeOfNode } = require('enzyme/build/Utils');
const shallowEqual = require('enzyme-shallow-equal');
const {
    displayNameOfNode,
    elementToTree: utilElementToTree,
    nodeTypeFromType: utilNodeTypeFromType,
    mapNativeEventNames,
    propFromEvent,
    assertDomAvailable,
    withSetStateAllowed,
...
const findCurrentFiberUsingSlowPath = require('./findCurrentFiberUsingSlowPath');
const detectFiberTags = require('./detectFiberTags');
    ...
    switch (node.tag) {
        ...
        case FiberTags.OffscreenComponent: {
            ...
            return {
                ...
                ref: node.ref,
                instance: null,
                rendered: childrenToTree(
                    nodeToHostNode(node.return.memoizedProps.children)
                ),
            };
        }
        default:
...
function getProviderDefaultValue(Provider) {
    ...
    throw new Error(
        'Enzyme Internal Error: can’t figure out how to get Provider’s default value'
    );
}
...
class ReactSeventeenAdapter extends EnzymeAdapter {
    ...
    createMountRenderer(options) {
        ...
        if (has(options, 'suspenseFallback')) {
            throw new TypeError(
                '`suspenseFallback` is not supported by the `mount` renderer'
            );
        }
        if (FiberTags === null) {
            // Requires DOM.
        ...
        return {
            ...
            unmount() {
                /*
                 * this code was having issues with unmount,
                 * triggering react-dom/act as suggested in this issue on the unofficial react 17 adaptor
                 * https://github.com/wojtekmaj/enzyme-adapter-react-17/issues/12#issuecomment-801806275
                 */
                const innerUnmount = () => {
                    ReactDOM.unmountComponentAtNode(domNode);
                    instance = null;
                };
                TestUtils.act(() => innerUnmount());
            },
            getNode() {
                if (!instance) {
                    return null;
    ...
    createShallowRenderer(options = {}) {
        ...
        if (
            typeof suspenseFallback !== 'undefined' &&
            typeof suspenseFallback !== 'boolean'
        ) {
            throw TypeError(
                '`options.suspenseFallback` should be boolean or undefined'
            );
        }
        let isDOM = false;
        let cachedNode = null;
            ...
            render(el, unmaskedContext, { providerValues = new Map() } = {}) {
                ...
                if (typeof el.type === 'string') {
                    ...
                    return withSetStateAllowed(() =>
                        renderElement({ ...el, type: MockProvider })
                    );
                    ...
                    return withSetStateAllowed(() =>
                        renderElement({ ...el, type: MockConsumer })
                    );
                } else {
                    isDOM = false;
                    let renderedEl = el;
                    if (isLazy(renderedEl)) {
                        throw TypeError(
                            '`React.lazy` is not supported by shallow rendering.'
                        );
                    }
                    ...
                    const context = getMaskedContext(
                    ...
                    return withSetStateAllowed(() =>
                        renderElement(renderedEl, context)
                    );
                }
            },
            unmount() {
